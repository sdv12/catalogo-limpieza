-- ============================================================
-- catalog_stats: se agrega deuda_total y clientes_con_deuda
-- ============================================================
  ...
  select case when public.is_catalog_member(p_catalog_id) then jsonb_build_object(
    ...
    'unidades_stock',
      (select coalesce(sum(pv.stock), 0)
        ...
        where pv.catalog_id = p_catalog_id and pv.is_deleted = false),
    'deuda_total',
      (select coalesce(sum(balance), 0) from public.customers
        where catalog_id = p_catalog_id and is_deleted = false and balance > 0),
    'clientes_con_deuda',
      (select count(*) from public.customers
        where catalog_id = p_catalog_id and is_deleted = false and balance > 0),
    'clientes_en_mora',
      (select count(*) from public.customers
        where catalog_id = p_catalog_id and is_deleted = false
          and balance > 0 and next_due_date < current_date)
  ) else '{}'::jsonb end;
$$;
