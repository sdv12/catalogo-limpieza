...
export const metadata = { title: "Proveedores — Catálogo" };
...
export default async function ProveedoresPage({
  ...
  let query = supabase
    .from("suppliers")
    .select(
      "id, name, doc_type, doc_number, contact_name, email, phone, city, payment_terms, is_active, is_deleted",
      { count: "exact" },
    )
  ...
  if (sp.q?.trim()) {
    ...
    query = query.or(
      `name.ilike.%${t}%,doc_number.ilike.%${t}%,contact_name.ilike.%${t}%,email.ilike.%${t}%`,
    );
  }
  ...
  const hacerHref = (cambios: SP) => {
    ...
    const s = q.toString();
    return `/panel/${slug}/proveedores${s ? `?${s}` : ""}`;
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-texto">Proveedores</h1>
        ...
        {!soloLectura && (
          <Link href={`/panel/${slug}/proveedores/nuevo`}>
            <Button>
              <Plus size={16} />
              Nuevo proveedor
            </Button>
      ...
      {(filas?.length ?? 0) === 0 ? (
        <EmptyState
          ...
          descripcion="Cargá tu primer proveedor o ajustá la búsqueda."
        />
      ) : (
        <>
          <SuppliersTable
